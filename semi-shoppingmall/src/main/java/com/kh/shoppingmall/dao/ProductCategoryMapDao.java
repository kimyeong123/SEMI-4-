...
public class ProductCategoryMapDao {
	...
	public void insert(int productNo, int categoryNo) {
		String sql = "insert into productCategoryMap(product_no, category_no) values(?, ?)";
		Object[] params = {productNo, categoryNo};
		jdbcTemplate.update(sql, params);
	}
	public boolean check(int productNo, int categoryNo) {
	    String sql = "select count(*) from productCategoryMap where product_no = ? and category_no = ?";
	    Object[] params = {productNo, categoryNo};
	    return jdbcTemplate.queryForObject(sql, Integer.class, params) > 0;
	}
	public boolean delete(int productNo, int categoryNo) {
		String sql = "delete productCategoryMap where product_no=? and category_no=?";
		Object[] params = {productNo, categoryNo};
		return jdbcTemplate.update(sql, params) > 0;
	}
	public List<Integer> selectCategoryNosByProductNo(int productNo) {
	    String sql = "select category_no from productCategoryMap where product_no = ? order by category_no asc"; 
	...
	public List<Integer> selectProductNosByCategoryNo(int categoryNo) {
	    String sql = "select product_no from productCategoryMap where category_no = ? order by product_no ASC";
